...
async def list_live_positions(
    ...
    """Open positions enriched with dashboard-only fields.
    ...
    Args:
        ...
        refresh: When true, pull intraday Polygon quotes (5-min cached per
                 underlying/expiry). Defaults to false so the initial page
                 load is instant using daily-batch prices — the book has
                 grown past the point where fetching every chain fits in
                 API Gateway's 30s timeout.
    """
    from app.paper_trading.live_view import enrich_position, fetch_live_quotes

    positions = await PaperPositionTable.list_open()

    # Scanner filter runs before quote fetch so `refresh=true` on a single
    # scanner stays within the timeout budget (~6 unique underlyings per
    # scanner slice, not 400+ across the whole book).
    if scanner and scanner.lower() != "all":
    ...
    quotes = await fetch_live_quotes(positions) if refresh else {}
...
async def live_positions_summary(refresh: bool = False) -> dict[str, Any]:
    """Portfolio header for the Active Positions dashboard.

    Returns total $ P&L, premium at risk, open count, attention counts,
    and the most recent quote timestamp across the book. See /positions/live
    for `refresh` semantics — default is off to keep the first render fast.
    """
